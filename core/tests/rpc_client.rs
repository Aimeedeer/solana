...
#[test]
// todo: this test is different from the doc example
...
#[test]
fn get_largets_accounts_with_config() -> ClientResult<()> {
    solana_logger::setup();
...
#[test]
#[ignore]
fn wait_for_max_stake() -> ClientResult<()> {
    solana_logger::setup();
    ...
    let commitment_config = CommitmentConfig::processed();
    let max_stake_percent = 33.0f32;
    let result = rpc_client.wait_for_max_stake(commitment_config, max_stake_percent);
    assert!(result.is_ok());
...
fn get_leader_schedule_with_config() -> ClientResult<()> {
    ...
    println!("get_leader_schedule with parameters");
    dbg!(&leader_schedule);
    ...
    println!("get_leader_schedule with empty parameters");
    dbg!(&leader_schedule);
    assert!(leader_schedule.is_some());
...
fn get_recent_performance_samples() -> ClientResult<()> {
    ...
    assert!(performance_samples.len() > 0);
